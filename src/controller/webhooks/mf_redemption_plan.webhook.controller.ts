...
export const handleMfRedemptionPlanWebhook = async (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
    ...
    try {
        ...
        if (!ALLOWED_EVENTS.has(event.type)) {
            logger.warn("Unsupported MF redemption plan webhook event", {
                event_id: event.id,
        ...
        logger.info("Received MF redemption plan webhook", {
            event_id: event.id,
            event_type: event.type,
        ...
        if (!claimed) {
            logger.info("Duplicate MF redemption plan webhook ignored", {
                event_id: event.id,
                event_type: event.type,
            });
        ...
        if (!trusted_object) {
            logger.warn(
                "MF redemption plan webhook could not fetch trusted FP object",
                {
                    event_id: event.id,
                    fp_id,
                },
        ...
        logger.info("MF redemption plan webhook processed successfully", {
            event_id: event.id,
            event_type: event.type,
            fp_id: updated.fp_id,
            state: updated.state,
        ...
        logger.error(
            "MF redemption plan webhook processing error:",
            error,
        );
